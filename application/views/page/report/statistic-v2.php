...
<div id="page-content">
	<div class="content-header">
		<div class="header-section">
			<h1>
				<i class="gi gi-briefcase"></i>Report Statistic V2<br>
			...
			<div class="modal-content">
				...
				<form id="edit_form" class="form-horizontal">
					<div class="modal-body">
						...
						<div class="form-group">
							<label class="col-md-3 control-label" for="server_edit">Server</label>
							<div class="col-md-9">
								<select class="form-control" name="server_edit" style="width: 100%" id="server_edit">
									<?php
									if ($server) {
										foreach ($server as $key => $value) {
											echo '<option  value="' . $value . '">' . strtoupper($value) . '</option>';
	...
	<script>
		...
		async function edit_data( id ) {
			...
			const select_server_drp = $( "#server_edit" ).data( 'kendoDropDownList' );
			...
			if ( dataOtp !== '' ) {
				...
				select_server_drp.value( dataOtp.server );
		...
		el_form_insert.on( 'submit', function ( e ) {
			...
			swalWithBootstrapButtons.fire( {
				...
				if ( result.isConfirmed ) {
					$.ajax( {
						url: '/otp-statistics-v2/insert.html',
						type: 'POST',
						dataType: 'json',
						data: {
						...
						success: function ( d ) {
							...
							//set timeout 1s and reload page
							setTimeout( function () {
		...
		el_form_edit.on( 'submit', function ( e ) {
			...
			let server_selected = $( "#server_edit" ).val();
			if ( isNullorEmpty( server_selected ) ) {
				alert_error( 'Empty server selected' );
				return;
			}
			...
			swalWithBootstrapButtons.fire( {
				...
				if ( result.isConfirmed ) {
					$.ajax( {
						url: '/otp-statistics-v2/update.html',
						type: 'POST',
						dataType: 'json',
						data: {
		...
		function delete_data( id ) {
			swalWithBootstrapButtons.fire( {
				...
				if ( result.isConfirmed ) {
					$.ajax( {
						url: "/otp-statistics-v2/delete.html",
						type: 'POST',
						dataType: 'json',
						data: {
							'id': id
		...
		async function getOne( id ) {
			...
			try {
				let response = await $.ajax( {
					url: '/otp-statistics-v2/get-one.html',
					type: 'POST',
					dataType: 'json',
					data: {
						'id': id
		...
		$( '#server_filter' ).on( 'change', function ( event ) {
			event.preventDefault();
			/* Act on the event */
			$( "#grid" ).data( "kendoGrid" ).dataSource.read();
		} );
		...
		function init_grid() {
			$( "#grid" ).kendoGrid( {
				dataSource: {
					transport: {
						read: {
							url: "<?= base_url('otp-statistics-v2/get-grid.html') ?>",
							dataType: "json",
							type: "POST",
							data: function ( a ) {
