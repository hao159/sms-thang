 ...
 **/
class Statistic_v2 extends NH_Controller
{

	public function __construct()
	{
		parent::__construct();
		$this->load->model('Otp_service_v2_model');
		//get server from distinct tbl_otp_services_v2
		$this->_server = $this->Otp_service_v2_model->getDistinct(null, 'server');
	...
	public function index()
	{
		...
		$data['temp'] = 'page/report/statistic-v2';
	...
	public function getgrid()
	{
		...
		try {
			...
			#get all
			$tmpData = $this->Otp_service_v2_model->getAllPaging($tmp);
	...
	public function getone()
	{
		...
		try {
			...
			$tmp_data = $this->Otp_service_v2_model->getOneByConditions($conditions);
			if (empty($tmp_data)) {
	...
	public function delete()
	{
		...
		try {
			...
			$tmp_data = $this->Otp_service_v2_model->delete($conditions);
	...
	public function update()
	{
		...
		try {
			...
			if (empty($tmp['id']) or empty($tmp['mem']) or !isset($tmp['count']) or !isset($tmp['server'])) {
			...
			$tmp_data = $this->Otp_service_v2_model->updateOne($data_update, $conditions);
			if (empty($tmp_data)) {
				throw new Exception('Invalid data id, or empty data');
			}
			$dataResponse->data = $tmp_data;
	...
	public function insert()
	{
		...
		try {
			...
			foreach ($tmp as $item) {
				...
				$tmpData = $this->Otp_service_v2_model->addNew($item);
				$count_result['ok']++;
