...
class Detail extends NH_Controller
{
	...
	{
		parent::__construct();
		$this->load->model('Otp_apple_model');
	}
	...
	public function getgrid()
	{
		...
		try {
			...
			if ($tmpData['total'] > 0) {
				...
				$pooling_timestamp = $this->Otp_apple_model->max_of_time();
	...
	public function delete()
	{
		...
		try {
			...
			$tmpData = $this->Otp_apple_model->delete_in($wheres, [
				'field' => 'mem',
				'values' => $mem_filter
			]);
			$dataResponse->status = StatusResponse::_SUCCESS;
